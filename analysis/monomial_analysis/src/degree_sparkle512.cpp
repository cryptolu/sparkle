 ...
 * degree_sparkle512.c
 *
 * Compile as: g++ -std=c++11 -Wall degree_sparkle512.cpp -o degree_sparkle512
 *
 ******************************************************************************/
...
// the function for which the ANF is checked (here Sparkle512)
int F() {
	uint64_t x0 = 0;
	uint64_t x1 = 0;
	uint64_t x2 = 0;
	...
	for (int i=0; i<64; i++) {
		...
		x4 = x4 ^ ((uint64_t)input[i+4*64]<<(63-i));
		x5 = x5 ^ ((uint64_t)input[i+5*64]<<(63-i));
		x6 = x6 ^ ((uint64_t)input[i+6*64]<<(63-i));
		x7 = x7 ^ ((uint64_t)input[i+7*64]<<(63-i));
	}
	for (int s=0; s<STEPS; s++)
	{
        ...
        x4r = x4 & 0xFFFFFFFF;
        x4l = (x4 >> 32) & 0xFFFFFFFF;
        x5r = x5 & 0xFFFFFFFF;
        x5l = (x5 >> 32) & 0xFFFFFFFF;
	x6r = x6 & 0xFFFFFFFF;
        x6l = (x6 >> 32) & 0xFFFFFFFF;
	x7r = x7 & 0xFFFFFFFF;
        x7l = (x7 >> 32) & 0xFFFFFFFF;
	...
	x3r=x4r;
        ...
        x4= ((uint64_t)(x4l) << 32) ^ (uint64_t)(x4r);
        x5= ((uint64_t)(x5l) << 32) ^ (uint64_t)(x5r);
        x6= ((uint64_t)(x6l) << 32) ^ (uint64_t)(x6r);        
	x7= ((uint64_t)(x7l) << 32) ^ (uint64_t)(x7r);
    }
	...
	for (int i=0; i<64; i++) {
		...
		output[i+4*64] = (x4 >> (63-i))&0x1;
		output[i+5*64] = (x5 >> (63-i))&0x1;
		output[i+6*64] = (x6 >> (63-i))&0x1;
		output[i+7*64] = (x7 >> (63-i))&0x1;
	}
...
int main(int argc, char* argv[])
{
   ...
   char filename[400];
   FILE *fp;
   sprintf(filename, "sparkle512_monomials%d_%d.dat", DEGREE, STEPS);
   fp = fopen(filename, "w");
   int nb[N_OUT];
