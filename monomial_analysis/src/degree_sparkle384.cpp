 ...
 * degree_sparkle384.c
 *
 * Compile as: g++ -std=c++11 -Wall degree_sparkle384.cpp -o degree_sparkle384
 *
 ******************************************************************************/
...
// the function for which the ANF is checked (here Sparkle384)
int F() {
	uint64_t x0 = 0;
	uint64_t x1 = 0;
	uint64_t x2 = 0;
...
int main(int argc, char* argv[])
{
   ...
   char filename[400];
   FILE *fp;
   sprintf(filename, "sparkle384_monomials%d_%d.dat", DEGREE, STEPS);
   fp = fopen(filename, "w");
   int nb[N_OUT];
